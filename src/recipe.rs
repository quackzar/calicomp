...
impl Ingredient {
    pub fn datasheet(&self) -> &Datasheet {
        match self {
            ...
            Ingredient::Generic { name: _, description: _, datasheet } => datasheet,
        }
    }
}
...
impl Recipe {
    ...
    pub fn calc_abv(&self) -> f64 {
        ...
        let milis = self.ingredients.iter()
            .map(|(volume, ingredient)| {
                volume.as_milliliters() * ingredient.datasheet.abv
            }).sum();
    ...
    pub fn calc_brix(&self) -> f64 {
        let milis = self.ingredients.iter()
            .map(|(volume, ingredient)| {
                volume.as_milliliters() * ingredient.datasheet.brix
            }).sum();
