// This is directly harvested from crossterm, since crossterm does
// not expose their parser, oh well.
//
// See: https://github.com/crossterm-rs/crossterm/blob/master/src/event/sys/unix/parse.rs
use std::io;
...
pub fn parse_event(
    ...
    match buffer[0] {
                ...
                match buffer[1] {
                    b'O' => {
                        if buffer.len() == 2 {
                            ...
                            match buffer[2] {
                                b'D' => {
                                    Ok(Some(Event::Key(KeyCode::Left.into())))
                                }
                                b'C' => Ok(Some(Event::Key(
                                    KeyCode::Right.into(),
                                ))),
                                b'A' => {
                                    Ok(Some(Event::Key(KeyCode::Up.into())))
                                }
                                b'B' => {
                                    Ok(Some(Event::Key(KeyCode::Down.into())))
                                }
                                b'H' => {
                                    Ok(Some(Event::Key(KeyCode::Home.into())))
                                }
                                b'F' => {
                                    Ok(Some(Event::Key(KeyCode::End.into())))
                                }
                                // F1-F4
                                val @ b'P'..=b'S' => Ok(Some(Event::Key(
        ...
        b'\n' if !crossterm::terminal::is_raw_mode_enabled()? => Ok(Some(
            Event::Key(KeyCode::Enter.into()),
        )),
        b'\t' => Ok(Some(Event::Key(KeyCode::Tab.into()))),
        b'\x7F' => Ok(Some(Event::Key(
        ...
        if let Some((special_key_code, state)) = translate_functional_key_code(codepoint) {
            ...
            (
                match c {
                    ...
                    // enter raw mode, we disable that, so \n no longer has any meaning - it's better to
                    // use Ctrl+J. Waiting to handle it here means it gets picked up later
                    '\n' if !crossterm::terminal::is_raw_mode_enabled()? => KeyCode::Enter,
                    '\t' => {
